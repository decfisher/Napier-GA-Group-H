    ...
    public ArrayList<Language> getLanguagePopularity()
    {
        try
        {
            ...
            String strSelect =
                    ...
                    ///        + "GROUP BY A.Language "
                    ///        + "ORDER BY 2 DESC;";
            "SELECT A.Language, ROUND(SUM((A.Percentage/100) * (B.Population/1000000)),2) AS TotalSpeakers, ROUND(SUM((A.Percentage/100) * (B.Population/1000000))/(SELECT SUM(Population/1000000) from country),2) AS PercentOfWorldPop "
                    + "FROM countrylanguage A "
