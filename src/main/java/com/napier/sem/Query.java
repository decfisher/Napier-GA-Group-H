...
public class Query {
    ...
    public ArrayList<Language> getLanguagePopularity()
    {
        try
        {
            ...
            // Create string for SQL statement
            String strSelect =
                    "SELECT A.Language, round(SUM((A.Percentage/100) * B.Population)/1000000,2) AS TotalSpeakers, concat(round((SUM(A.Percentage * B.Population)/SUM(B.Population)),2),'%') AS PercentOfWorldPop "
